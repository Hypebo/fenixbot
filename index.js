...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Rafael\n' 
            + 'ORG: Pengembang XBot;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=5581212020:+55 (62) 8121-2020\n' 
            + 'END:VCARD' 
prefix = '!'
blocked = []          
...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '❬🤙❭AGUARDE  UM POUCO MACACO KKK ❬🤙❭',
				success: '️❬ ✔ ❭ PRONTO',
				error: {
...
case 'lofi':
			...
			case 'mapa': 
				...
				case 'audio':
					...
					rano = getRandom('.ogg')
					dtt.length > 300
					? reply('texto muito grande macaco😤')
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							fs.unlinkSync(ranm)
							buff = fs.readFileSync(rano)
							if (err) return reply('Desculpa macaco, eu falhei nessa :( ')
       ...
       client.sendMessage(from, 'Esse número é do meu criador :) ',MessageType.text, { quoted: mek} )
           break    
           case 'setname':
           case 'mudarnome':
                if (!isGroup) return reply(mess.only.group)
                ...
                client.sendMessage(from, 'Nome alterado com sucesso macaco', text, {quoted: mek})
                break
                case 'setdesc':
                case 'mudardesc':
                if (!isGroup) return reply(mess.only.group)
                ...
                client.sendMessage(from, 'descrição alterada com sucesso macaco', text, {quoted: mek})
                break
           case 'demote':
           case 'rebaixar':
					if (!isGroup) return reply(mess.only.group)
